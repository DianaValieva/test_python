...
def test_yashaka(open_google):
    browser.element('[name="q"]').should(be.blank).type('yashaka/selene').press_enter()
    assert browser.element('[id="search"]').should(
        have.text('Selene - User-oriented Web UI browser tests in Python')), "Подходящая страница не найдена"

def test_wrong_string(open_google):
    browser.element('[name="q"]').should(be.blank).type('vmbndiovjsokjk').press_enter()
    assert browser.element('#extabar #result-stats').should(
